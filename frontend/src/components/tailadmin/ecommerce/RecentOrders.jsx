export default function RecentOrders({ invoices = [] }) {
  const tableData = invoices.map((inv) => ({
    id: inv.id,
    name: inv.invoiceNumber,
    category: inv.customer,
    price: new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'INR' }).format(inv.totalAmount || 0),
    status: inv.status === 'paid' ? 'Delivered' : inv.status === 'overdue' ? 'Canceled' : 'Pending',
  }))
